...
HttpRequestInput::HttpRequestInput() { initialize(); }
...
void HttpRequestInput::add_file(QString variable_name, QString local_filename,
...
QString HttpRequestWorker::http_attribute_encode(QString attribute_name,
                                                 QString input) {
  ...
  for (int i = 0; i < input_c.length(); i++) {
    ...
    if ((c >= '0' && c <= '9') || (c >= 'A' && c <= 'Z') ||
      ...
      result_utf8 +=
          "%" + QString::number(static_cast<unsigned char>(input_c.at(i)), 16)
                    .toUpper();
    }
  }
...
void HttpRequestWorker::execute(HttpRequestInput *input) {
  ...
  if (input->var_layout == NOT_SET) {
    input->var_layout =
        input->http_method == "GET" || input->http_method == "HEAD"
            ? ADDRESS
            : URL_ENCODED;
  }
  ...
  if (input->var_layout == ADDRESS || input->var_layout == URL_ENCODED) {
    ...
    // add files
    for (QList<HttpRequestInputFileElement>::iterator file_info =
             input->files.begin();
      ...
      if (file_info->local_filename == NULL ||
          ...
          file_info->variable_name.isEmpty() || !fi.exists() || !fi.isFile() ||
          !fi.isReadable()) {
      ...
      if (file_info->request_filename == NULL ||
          file_info->request_filename.isEmpty()) {
      ...
      request_content.append(
          QString("Content-Disposition: form-data; %1; %2")
              .arg(http_attribute_encode("name", file_info->variable_name),
                   http_attribute_encode("filename",
